/* ============================================================
   BARNACLES — weather & atmosphere (clock-aware)
   Modes: storm (rain+lightning, scaled by time of day), deep
   (slow dust), cabin (embers), forge (sparks), water (bubbles),
   shore (haze). Reads BRN.Clock so days are calmer & brighter.
   ============================================================ */
(function () {
  'use strict';
  ...
  function storm() { return (window.BRN && BRN.Clock) ? BRN.Clock.storm : 1; }
  ...
  function seed() {
    W.rain = [];
    for (var i = 0; i < 340; i++) W.rain.push({ x: rand(0, W.w), y: rand(0, W.h), len: rand(12, 30) * devicePixelRatio, sp: rand(14, 26) * devicePixelRatio, w: rand(.5, 1.4) * devicePixelRatio });
    W.motes = [];
  ...
  function drawRain(base) {
    var intensity = base * (0.25 + 0.75 * storm()); // thinner rain by day
    if (intensity < 0.03) return;
    var c = W.ctx;
    c.strokeStyle = 'rgba(180,205,225,' + (0.16 * intensity) + ')';
    c.lineCap = 'round';
    var count = Math.floor(W.rain.length * Math.min(1, intensity + 0.15));
  ...
  function loop() {
    ...
    if (W.mode === 'storm') { drawRain(1); drawMotes('rgba(200,210,220,A)', 0.4); maybeStrike(); }
    else if (W.mode === 'cabin') { drawMotes('rgba(230,180,90,A)', 0.5); drawRain(0.25); }
    else if (W.mode === 'deep') { drawMotes('rgba(150,170,180,A)', 0.25); }
    else if (W.mode === 'forge') { drawMotes('rgba(255,150,60,A)', 0.9); }
    else if (W.mode === 'water') { caustics(); drawBubbles(); }
    else if (W.mode === 'shore') { drawMotes('rgba(220,225,210,A)', 0.3); drawRain(0.1); }
  ...
  function maybeStrike() {
    if (W.t > W._nextStrike) {
      // far rarer in daylight
      var gap = rand(280, 900) / Math.max(0.15, storm());
      W._nextStrike = W.t + gap;
      if (storm() > 0.35) W.strike();
    }
  }

  W.strike = function () {
    if (W.flashEl) {
      W.flashEl.style.opacity = '';
      W.flashEl.classList.remove('flash');
      void W.flashEl.offsetWidth;
